...
export const AnimatedList = React.memo(
  ({ children, className, delay = 1000, loop = true, ...props }: AnimatedListProps) => {
    ...
    const itemsToShow = useMemo(() => {
      ...
      const maxItems = Math.min(index + 1, childrenArray.length + 3)

      for (let i = 0; i < maxItems; i++) {
        const itemIndex = (index - i) % childrenArray.length
        const actualIndex = itemIndex < 0 ? itemIndex + childrenArray.length : itemIndex
        const child = childrenArray[actualIndex]
        items.push({
          key: `${(child as React.ReactElement)?.key ?? actualIndex}-${index - i}`,
          element: child,
        })
      }
      return items
